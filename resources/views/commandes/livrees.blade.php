        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" action="{{ route('commandes.livrees') }}" class="bg-white rounded-2xl border border-stade-950/5 shadow-sm p-4 sm:p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
                    ...
                    <div>
                        ...
                        <select id="qualite" name="qualite"
                            class="w-full border bg-white border-stade-700/15 text-stade-950 rounded-lg shadow-sm py-2.5 pl-3 pr-9 focus:outline-none focus:border-or-500 focus:ring-2 focus:ring-or-500/30">
                            <option value="">Toutes les qualités</option>
                            @foreach(\App\Models\Commande::QUALITES as $valeur => $label)
                                <option value="{{ $valeur }}" @selected($qualite === $valeur)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="type_article" class="sr-only">Type d'article</label>
                        <select id="type_article" name="type_article"
                            class="w-full border bg-white border-stade-700/15 text-stade-950 rounded-lg shadow-sm py-2.5 pl-3 pr-9 focus:outline-none focus:border-or-500 focus:ring-2 focus:ring-or-500/30">
                            <option value="">Tous les types</option>
                            @foreach(\App\Models\Commande::TYPES_ARTICLE as $valeur => $label)
                                <option value="{{ $valeur }}" @selected($typeArticle === $valeur)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="mt-3 flex items-center justify-between gap-3">
                    <div>
                        @if($client || $qualite || $typeArticle || $dateDebut || $dateFin)
                        ...
                        @foreach($commandes as $commande)
                            <li class="px-4 py-4">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <span class="shrink-0 text-xs text-stade-600 whitespace-nowrap">
                                        {{ $commande->date_livraison_effective?->format('d/m/Y') }}
                                    </span>
                                </div>

                                <p class="mt-2.5 text-sm text-stade-700">
                                    {{ $commande->type_article }}
                                    <span class="text-stade-600/60">&middot; Qualité {{ $commande->qualite }} &middot; Qté {{ $commande->quantite }}</span>
                                </p>

                                <div class="mt-3 flex items-center gap-5">
                                    <a href="{{ route('commandes.show', $commande) }}" class="text-sm font-semibold text-stade-600 hover:text-stade-950">
                                        Détails
                                    </a>
                                    <a href="{{ route('commandes.bon-livraison', $commande) }}" target="_blank" class="text-sm font-semibold text-or-600 hover:text-or-700">
                                        Bon de livraison
                                    </a>
                                </div>
                            </li>

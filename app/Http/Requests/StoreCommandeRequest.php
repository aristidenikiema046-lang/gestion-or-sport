...
class StoreCommandeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'articles.*.type_article' => ['required', Rule::in(array_keys(Commande::TYPES_ARTICLE))],
            'articles.*.qualite' => ['required', Rule::in(array_keys(Commande::QUALITES))],
            'articles.*.modele' => ['required', Rule::in(array_keys(Commande::MODELES))],
            'articles.*.nom_equipe' => ['nullable', 'string', 'max:255'],
    ...
    public function attributes(): array
    {
        return [
            ...
            'articles.*.type_article' => "type d'article",
            'articles.*.qualite' => 'qualité',
            'articles.*.modele' => 'modèle',
            'articles.*.nom_equipe' => "nom de l'équipe",
